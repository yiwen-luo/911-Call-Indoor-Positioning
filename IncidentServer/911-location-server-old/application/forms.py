...
class EnterAddrInfo(Form):
    room = TextField('Room')
    floor = TextField('Floor', validators=[validators.required(), validators.Regexp('^\d$',message=u'Enter a number only')])
    building = TextField("Building Name")
    district = TextField("District Name")
    city = TextField("City")
